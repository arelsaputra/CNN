...
def predict():
    ...
    class_names = ['Tumbuhan Dikotil', 'Tumbuhan Monokotil']
    # result = {
    ...
    return render_template("classifications.html", img_path=predict_image_path,
                           predictionnasnet=class_names[np.argmax(
                               prediction_array_nasnet)],
                           confidencenasnet='{:2.0f}%'.format(
                               100 * np.max(prediction_array_nasnet)),
                           predictionvgg=class_names[np.argmax(
                               prediction_array_vgg)],
                           confidencvgg='{:2.0f}%'.format(
                               100 * np.max(prediction_array_vgg)),
                           predictionxception=class_names[np.argmax(
                               prediction_array_xception)],
                           confidenceexception='{:2.0f}%'.format(
                               100 * np.max(prediction_array_xception)),
                           predictioncnn=class_names[np.argmax(
                               prediction_array_cnn)],
                           confidencecnn='{:2.0f}%'.format(
                               100 * np.max(prediction_array_cnn))
                           )
